document.addEventListener('DOMContentLoaded', () => {
    ...
    // Dán vào file home.js, thay thế logic cũ của category slider

    // =============================================================
    // LOGIC CHO CATEGORY SLIDER (LAYOUT 2x2 TRÊN MOBILE)
    // =============================================================
    ...
    if (categoryGridContainer && prevBtn && nextBtn && categoryPagination) {
        ...
        const itemsPerPage = 4; // 4 mục mỗi trang (2x2)
        const totalPages = Math.ceil(categories.length / itemsPerPage);
        let currentPage = 0;

        // Render cấu trúc slider
        function renderSlider() {
            categoryGridContainer.innerHTML = ''; // Xóa nội dung cũ
        ...
        // Cập nhật trạng thái (nút và dots)
        ...
        // Hàm cuộn đến một trang cụ thể
        function scrollToPage(pageIndex) {
            const pageElements = categoryGridContainer.querySelectorAll('.category-page');
            if (pageElements[pageIndex]) {
                pageElements[pageIndex].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'start' });
            }
        }
        ...
        // Lắng nghe sự kiện cuộn để cập nhật trạng thái khi người dùng vuốt
        const scrollObserver = new IntersectionObserver((entries) => {
            ...
            root: categoryGridContainer,
            threshold: 0.6 // Kích hoạt khi 60% trang hiển thị
        });
        ...
        createPagination();
        updateState();
        // Gắn observer cho tất cả các trang
        categoryGridContainer.querySelectorAll('.category-page').forEach(page => {
